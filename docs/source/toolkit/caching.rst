...
* Dependent token cache: Dependent tokens are used when the Action Provider needs to make calls to other services on behalf of the user making a request. Action Provider Tools performs a dependent token grant when ``AuthState.get_authorizer_for_scope`` is called. A hash of the caller's access token is used as the key for the cache lookup. A value in the cache has a lifetime of 47 hours, which is just under the 48-hour lifetime of an access token.

* Group Membership cache: Invoking the Groups service requires a dependent token, which will be retrieved when ``AuthState.get_authorizer_for_scope`` is called. A hash of the caller's access token is used as a key to the group membership cache. A value in the cache has a lifetime of 5 minutes, and, thus, after 5 minutes, the Groups service will be invoked to get group membership even if the access token to be used for accessing the Groups service is still in the Dependent token cache.
